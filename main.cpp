...
// Extract allowed_docs integer list from JSON, returns std::set<int>
std::set<int> extractAllowedDocs(const std::string& body) {
    std::set<int> ids;
    size_t p = body.find("\"allowed_docs\"");
    if (p == std::string::npos) return ids;
    p = body.find('[', p);
    if (p == std::string::npos) return ids;
    size_t e = body.find(']', p);
    if (e == std::string::npos) return ids;
    std::string arrStr = body.substr(p + 1, e - p - 1);
    ...
    while (std::getline(ss, t, ',')) {
        try {
            ...
            if (start != std::string::npos && end != std::string::npos) {
                ids.insert(std::stoi(t.substr(start, end - start + 1)));
            }
        } catch (...) {}
    }
    return ids;
...
public:
    ...
    std::vector<std::pair<float, DocItem>> search(
        const std::vector<float>& q, int k, const std::set<int>& allowed = {}, float max_dist = 0.7f)
    {
        std::lock_guard<std::mutex> lk(mu);
        if (store.empty()) return {};
        auto raw = (store.size() < 10)
                   ? bf.knn(q, allowed.empty() ? k : 100, cosine)
                   : hnsw.knn(q, allowed.empty() ? k : 100, 100, cosine);
        std::vector<std::pair<float, DocItem>> out;
        for (auto& [d, id] : raw) {
            if (store.count(id) && d <= max_dist) {
                if (allowed.empty() || allowed.count(id)) {
...
int main() {
    ...
    std::cout << "=== NeuroVault Engine ===" << std::endl;
    ...
    svr.Post("/doc/search", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto allowed = extractAllowedDocs(req.body);
        auto hits = docDB.search(qEmb, k, allowed);
